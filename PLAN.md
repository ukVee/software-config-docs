...
Last updated: 2026-07-12 (session 9 — ran before S8, which ukv writes by hand)
...
| 7 | handoffs/session-07-internals.md | Codebase architecture + subsystem pages | done 2026-07-12 — all five Codebase pages written from the crates (three parallel crate-reader passes over keeperd/fuse, vcs/store, vault/net): architecture (15-crate map, the two boundaries = frontend-neutral cores + daemon-owns-all-writes, the shared write pipeline + ASCII data-flow, on-disk shape), daemon-and-fuse (born-in-FUSE, the two-source/one-accumulator watcher + intent classifier, read redaction + write overlay, lock/unlock + relock token, the JSON-Lines IPC surface), vcs (content-addressed ciphertext store + sqlite index, JCS-canonical trees + signed commits, the closed intent enum, log/fsck, "replacing git" concretely), vault (two layers/two threat models, the KEK→M→subkey hierarchy, convergent-encryption trade-off, BIP39 recovery + passphrase rotation, device identity + the three-doors trust split), status (dated snapshot: shipped-on-main vs M5c-in-development-on-branch vs not-started, ~1,100 #[test] fns). Honesty tiering: M5c union-mount/shared-subtrees flagged unmerged+hardening (repo checked out on feat/m5c-union-mount); overlap rule honored (crypto.md = facts, vault.md = how/why). Build green (28 pages). |
| 8 | handoffs/session-08-story.md | Why + evolution (ukv writes, Claude edits) | deferred — ukv writes these by hand in his own voice (grounding rule 5); intentionally skipped when S9 ran. `story/why.md` + `story/evolution.md` are graceful "coming soon" placeholders (link to garden/today + vision-roadmap), not ghost-written. |
| 9 | handoffs/session-09-readme-launch.md | Splash polish, code-repo README rewrite, launch checklist | done 2026-07-12 — ran ahead of S8 (skipped by request). Splash: real hero + CardGrid into the six main areas, honest single-author status caution, garden-first pitch, TODO/"most pages are stubs" removed. Full-site pass: build green (28 pages), no demo remnants, story stubs reworded to public placeholders. Code-repo README shrunk 331→~110 lines (pitch, prominent docs link, reconciled status banner — M5a/M5b/growlight now shown shipped-on-main + M5c in-dev-on-branch, fixing the "README trails the code" drift, condensed quickstart, license); **left UNCOMMITTED on `feat/m5c-union-mount` — must land on `main` (see LAUNCH.md), not merge the M5c branch.** `LAUNCH.md` written (not executed): repo-name is `software-config-docs` per astro base, private-remote creation, gated flip-public → dispatch workflow → verify → land README. |
